...
self.addEventListener('fetch', (event) => {
  ...
  // 1. Data-related / API or dynamic assets: Stale-While-Revalidate strategy
    ...
    event.respondWith(
      caches.match(request).then((cachedResponse) => {
        ...
        return fetch(request).then((networkResponse) => {
          ...
          // Offline fallback for static asset if needed
        });
      })
    );
    return;
  ...
  // 3. Default (HTML / Navigation): Stale-While-Revalidate with index.html fallback
  event.respondWith(
    caches.match(request).then((cachedResponse) => {
      const fetchPromise = fetch(request).then((networkResponse) => {
        if (networkResponse && networkResponse.status === 200) {
